...
if [[ "${MODE}" == "standalone" ]]; then
    JAVA_OPT="${JAVA_OPT} -Dfile.encoding=UTF-8 -server -Xms256m -Xmx512m"
    JAVA_OPT="${JAVA_OPT} -Dnacos.standalone=true"
else
    JAVA_OPT="${JAVA_OPT} -Dfile.encoding=UTF-8 -server -Xms2g -Xmx2g -Xmn1g -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=320m"
...
nohup echo "nacos is starting，you can check the ${BASE_DIR}/logs/start.out"
